...
export default function ConsumerProfile() {
  ...
  useEffect(async () => {
    try {
      await axios.get(`http://localhost:9090/consumer/viewConsumerDetails/${localStorage.getItem('UserId')}`)
        .then(response => {
    ...
    console.log(consumerData.address);
  ...
  const onSubmit = (data) => {
    ...
    try {
      axios.put(`http://localhost:9090/consumer/updateConsumerDetails/${localStorage.getItem('UserId')}`, consumerUpdatedData)
        .then(response => {
